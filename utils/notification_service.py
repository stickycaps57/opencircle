...
class NotificationService:
    ...
    def notify_organization_members_event_deleted(
        self, organization_id: int, event_title: str, organization_name: str
    ) -> bool:
        """Notify all organization members when an event is deleted."""
        try:
            # Get all approved members of the organization
            members_query = (
                self.session.query(
            ...
            message = f"The event '{event_title}' organized by {organization_name} has been cancelled."
            ...
            print(f"Error notifying members about event deletion: {e}")
            return False
        except Exception as e:
