    ...
    try:
        ...
        for share in shares_result:
            ...
            if content_details and sharer_result:
                ...
                auth_user_rsvp = None
                # If this is an event share and we have an authenticated user, get their RSVP status
                if share_data["content_type"] == 2 and account_id:
                    rsvp_stmt = select(
                        table["rsvp"].c.id,
                        table["rsvp"].c.status
                    ).where(
